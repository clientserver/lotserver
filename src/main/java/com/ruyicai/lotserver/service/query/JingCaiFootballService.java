 ...
 */
@Service
public class JingCaiFootballService {

	private Logger logger = Logger.getLogger(JingCaiFootballService.class);
	...
	public String getDataAnalysis(ClientInfo clientInfo) {
		...
		try {
			...
			String url = propertiesUtil.getDataAnalysisUrl() + "select/getInfo";
			String result = HttpUtil.sendRequestByPost(url, paramStr.toString(), true);
			//logger.info("获取竞彩足球数据分析返回:"+result+",paramStr:"+paramStr.toString());
	...
	public String getImmediateScore(ClientInfo clientInfo) {
		...
		try {
			...
			String url = propertiesUtil.getDataAnalysisUrl() + "select/getImmediateScores";
			String result = HttpUtil.sendRequestByPost(url, paramStr.toString(), true);
			//logger.info("获取竞彩足球即时比分列表返回:"+result+",paramStr:"+paramStr.toString());
			if (Tools.isEmpty(result)) { // 如果返回空,参数错误
				return Tools.paramError(clientInfo.getImei());
			}
			JSONObject fromObject = JSONObject.fromObject(result);
	...
	public String getImmediateScoreDetail(ClientInfo clientInfo) {
		...
		try {
			...
			String url = propertiesUtil.getDataAnalysisUrl() + "select/getImmediateScore";
			String result = HttpUtil.sendRequestByPost(url, paramStr.toString(), true);
			...
			responseJson.put(Constants.error_code, ErrorCode.fail.value());
			responseJson.put(Constants.message, "系统繁忙");
			logger.error("查询竞彩足球即时比分详细发生异常", e);
		}
		return responseJson.toString();
	}
